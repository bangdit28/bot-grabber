...
import time, os, threading, re, json
...
def grab_sms_mnit():
    ...
    while True:
        # KITA PAKSA CEK TANGGAL 08 DAN 09 (Biar gak miss gara-gara beda jam server)
            ...
            try:
                ...
                if res.status_code == 200:
                    ...
                    for item in items:
                        ...
                        if code and num:
                            # Bersihkan tag HTML dari kode OTP
                            clean_code = re.sub('<[^<]+?>', '', str(code)).strip()
                            if not clean_code: continue
                            
                            uid = f"{num}_{clean_code}"
                            if uid not in done_ids:
                                print(f"✅ KODE KETANGKAP: {num} -> {clean_code}")
                                msg_content = f"<#> {clean_code} is your Facebook code"
                                
                                # 1. KIRIM KE FIREBASE (Path: messages)
                                # Pastikan liveSms formatnya sama dengan yang ada di active_numbers
        ...
        try:
            ...
            if perintah:
                for req_id, val in perintah.items():
                    ...
                    if nomor:
                        # SIMPAN KE /active_numbers biar Web lo nampilin
